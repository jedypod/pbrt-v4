...
TEST(IntSlice, StaticArrayConversion) {
  int array[20];
  IntVec vec;
  Fill(&vec, sizeof(array) / sizeof(array[0]));
...
TEST(IntSlice, ConstStaticArrayConversion) {
  IntVec vec;
  Fill(&vec, sizeof(test_const_array) / sizeof(test_const_array[0]));
...
TEST(MutableIntSlice, StaticArrayConversion) {
  ...
  static_cast<void>(v);
  TestImplicitConversion(array, array, sizeof(array) / sizeof(array[0]));
}
