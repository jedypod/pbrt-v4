...
void CPURender(ParsedScene &parsedScene) {
            ...
            for (auto &s : shapes) {
                ...
                if (sh.lightIndex != -1) {
                    ...
                    LightHandle area = LightHandle::CreateArea(
                        ...
                        AnimatedTransform(*sh.worldFromObject), mi, s,
                        &areaLightEntity.loc, Allocator{} /* FIXME LEAK */);
                    areaHandle = area;
                    if (area)
    ...
    for (const auto &inst : parsedScene.instances) {
        ...
        if (inst.worldFromInstance)
            primitives.push_back(
                new TransformedPrimitive(iter->second, inst.worldFromInstance));
        else
            primitives.push_back(
                new AnimatedPrimitive(iter->second, inst.worldFromInstanceAnim));
    }
