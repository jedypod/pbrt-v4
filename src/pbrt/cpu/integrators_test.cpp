...
std::vector<TestScene> GetScenes() {
    ...
    static Transform identity;
    {
        ...
        std::vector<LightHandle> lights;
        lights.push_back(new PointLight(AnimatedTransform(identity), MediumInterface(),
                                        &I, Allocator()));
...

    {
        ...
        LightHandle areaLight = new DiffuseAreaLight(
            AnimatedTransform(identity), MediumInterface(), &Le, 1.f, sphere,
            pstd::optional<Image>{}, nullptr, false, Allocator());
...
std::vector<TestIntegrator> GetIntegrators() {
    ...
    for (const auto &scene : GetScenes()) {
        ...
        for (auto &sampler : GetSamplers(resolution)) {
            ...
            RGBFilm *film =
                new RGBFilm(resolution, Bounds2i(Point2i(0, 0), resolution), filter, 1.,
