...
int cat(int argc, char *argv[]) {
    ...
    for (int i = 0; i < argc; ++i) {
        ...
        pstd::optional<ImageAndMetadata> im = Image::Read(argv[i]);
        if (!im) {
...
int average(int argc, char *argv[]) {
    ...
    ParallelFor(0, filenames.size(), [&](size_t i) {
        pstd::optional<ImageAndMetadata> imRead = Image::Read(filenames[i]);
        if (!imRead) {
            fprintf(stderr, "%s: unable to read image.\n", filenames[i].c_str());
            failed = true;
            return;
...
int error(int argc, char *argv[]) {
    ...
    auto ref = Image::Read(referenceFile);
    if (!ref)
        return 1;
...
int whitebalance(int argc, char *argv[]) {
    ...
    SquareMatrix<3> ccMatrix;
    if (!illuminant.empty()) {
        std::string name = "stdillum-" + illuminant;
        SpectrumHandle illum = SPDs::GetNamed(name);
        if (!illum) {
...
int makeenv(int argc, char *argv[]) {
    ...
    pstd::optional<ImageAndMetadata> latlong = Image::Read(inFilename);
    if (!latlong)
        return 1;
...
int main(int argc, char *argv[]) {
    ...
    if (strcmp(argv[1], "average") == 0)
        ...
        auto imRead = Image::Read(filename);
        if (!imRead) {
            fprintf(stderr, "%s: unable to read image\n", filename.c_str());
            return 1;
        }
