...
//   #include <pbrt/core/tiny_obj_loader.h>
//
...
int main(int argc, char *argv[]) {
    ...
    for (int i = 1; i < argc; ++i) {
        if ((strcmp(argv[i], "--help") == 0) || (strcmp(argv[i], "-h") == 0))
            usage();
        else if (strcmp(argv[i], "--ptexquads") == 0)
            ptexQuads = true;
        else if (objFilename == nullptr)
            objFilename = argv[i];
        else if (pbrtFilename == nullptr)
            pbrtFilename = argv[i];
        else
            usage();
    }
    if (pbrtFilename == nullptr)
        usage();
