...
int main(int argc, char *argv[]) {
    ...
    if (!options.quiet && !format && !toPly && !options.upgrade) {
        ...
        printf("*** DEBUG BUILD ***\n");
#endif  // !NDEBUG
        printf("Copyright (c)1998-2020 Matt Pharr, Greg Humphreys, and Wenzel "
               "Jakob.\n");
...
#ifdef PBRT_BUILD_GPU_RENDERER
        ParsedScene scene(gpu ? gpuMemoryAllocator : Allocator());
#else
        // WAR most vexing parse
        ParsedScene scene{Allocator()};
#endif
