...
TEST(Vector, AngleBetween) {
    ...
    EXPECT_LT(std::abs(AngleBetween(Vector3f(1, 0, 0), Vector3f(0, 1, 0))) - Pi / 2, 1e-7);
...
//CO    LOG(WARNING) << StringPrintf("naive %f (err %f), abet %f (err %f) old %f (err %f)",
//CO                                 naive, std::abs(naive - precise) / precise,
//CO                                 abet, std::abs(abet - precise) / precise,
//CO                                 old, std::abs(old - precise) / precise);
}
...
TEST(CylindricalCoordinates, ToBasics) {
    {
        Point2f c = ToCylindrical(Vector3f(0, 0, 1));
        EXPECT_EQ(1, c[0]);
        EXPECT_EQ(0, c[1]);
    }
    {
    ...
    {
        Point2f c = ToCylindrical(Vector3f(1, 0, 0));
        EXPECT_EQ(0, c[0]);
        EXPECT_EQ(0, c[1]);
    }
    {
        Point2f c = ToCylindrical(Vector3f(0, 1, 0));
        EXPECT_EQ(0, c[0]);
        EXPECT_EQ(Pi / 2, c[1]);
    }
    {
    ...
    }
    {
        Point2f c = ToCylindrical(Vector3f(0, -1, 0));
        EXPECT_EQ(0, c[0]);
        EXPECT_EQ(Pi * 3 / 2, c[1]);
    }
}

TEST(CylindricalCoordinates, FromBasics) {
    {
        Vector3f v = FromCylindrical(Point2f(1, 0));
        EXPECT_EQ(Vector3f(0, 0, 1), v) << v;
    }
    {
        Vector3f v = FromCylindrical(Point2f(1, 0.1245));
        EXPECT_EQ(Vector3f(0, 0, 1), v) << v;
    }
    {
        Vector3f v = FromCylindrical(Point2f(-1, 0));
    ...
    {
        Vector3f v = FromCylindrical(Point2f(-1, 0.734));
        EXPECT_EQ(Vector3f(0, 0, -1), v) << v;
    }
    {
        Vector3f v = FromCylindrical(Point2f(0, 0));
        EXPECT_EQ(Vector3f(1, 0, 0), v) << v;
    }
    {
        Vector3f v = FromCylindrical(Point2f(0, Pi / 2));
        EXPECT_GT(Dot(Vector3f(0, 1, 0), v), 0.9999) << v;
    }
    {
        Vector3f v = FromCylindrical(Point2f(0, Pi));
        EXPECT_GT(Dot(Vector3f(-1, 0, 0), v), 0.9999) << v;
    }
    {
        Vector3f v = FromCylindrical(Point2f(0, Pi * 3 / 2));
        EXPECT_GT(Dot(Vector3f(0, -1, 0), v), 0.9999) << v;
    }
}

TEST(CylindricalCoordinates, Randoms) {
    for (Point2f u : Uniform2D(100)) {
        Vector3f v = SampleUniformSphere(u);
        Point2f c = ToCylindrical(v);
        Vector3f vp = FromCylindrical(c);
...
TEST(DirectionCone, UnionBasics) {
    ...
    Vector3f w(1, .5, -.25);
    c = Union(DirectionCone(w, std::cos(.12)),
              DirectionCone(w, std::cos(.03)));
    EXPECT_EQ(Normalize(w), c.w);
    EXPECT_FLOAT_EQ(std::cos(.12), c.cosTheta);

    // Same direction, second wider
    c = Union(DirectionCone(w, std::cos(.1)),
              DirectionCone(w, std::cos(.2)));
    EXPECT_EQ(Normalize(w), c.w);
...
TEST(DirectionCone, UnionRandoms) {
    ...
    for (int i = 0; i < 100; ++i) {
        ...
        for (int j = 0; j < 100; ++j) {
            Vector3f w = SampleUniformSphere({rng.Uniform<Float>(),
                        rng.Uniform<Float>()});
            if (Inside(a, w) || Inside(b, w))
                EXPECT_TRUE(Inside(c, w)) << "a: " << a << ", b: " <<
                b << ", union: " << c << ", w: " << w;
        }
    }
}
...
TEST(DirectionCone, BoundBounds) {
    ...
    // It's fine for it to be a bit big (as it is in practice due to approximations
    // for performance), but it shouldn't be too big.
    EXPECT_LT(theta, 1.1 * precise);

    RNG rng(512);
    for (int i = 0; i < 1000; ++i) {
        Bounds3f b(Point3f(Lerp(rng.Uniform<Float>(), -1, 1),
                           Lerp(rng.Uniform<Float>(), -1, 1),
                           Lerp(rng.Uniform<Float>(), -1, 1)),
                   Point3f(Lerp(rng.Uniform<Float>(), -1, 1),
                           Lerp(rng.Uniform<Float>(), -1, 1),
                           Lerp(rng.Uniform<Float>(), -1, 1)));

        Point3f p(Lerp(rng.Uniform<Float>(), -4, 4),
...
TEST(DirectionCone, VectorInCone) {
    ...
    for (int i = 0; i < 100; ++i) {
        ...
        for (int j = 0; j < 100; ++j) {
            Vector3f wRandom = SampleUniformSphere({rng.Uniform<Float>(), rng.Uniform<Float>()});
    ...
    for (int i = 0; i < 100; ++i) {
        ...
        for (int y = 0; y < sqrtN; ++y) {
            for (int x = 0; x < sqrtN; ++x) {
                Point2f u((x + rng.Uniform<Float>()) / sqrtN,
                          (y + rng.Uniform<Float>()) / sqrtN);
                Vector3f v;
                if (sampleSphere)
        ...
        EXPECT_TRUE(estA >= .975 * area && estA <= 1.025 * area) <<
            "Area " << area << ", estimate " << estA << ", va = " << a <<
            ", vb = " << b << ", vc = " << c;
    }
}
