...
RGBColorSpace::RGBColorSpace(Point2f r, Point2f g, Point2f b, Point2f w,
    ...
    /* Try n:
       ...
       4. We know that the X value is proportional to the sum of the Rx, Gx, Bx
          ...
          [ Y ] = [ ry gy by ] [ 0 Cg  0 ] [ 1 ]
          [ Z ] = [ rz gz bz ] [ 0  0 Cb ] [ 1 ]
       5. Now solve for Cr/Cg/Cg: multiply both sides by the inverse of the
       rx... matrix and we've got Cr, Cg, and Cb. Now multiply to get the RGB ->
       XYZ matrix. Invert that and we can go the other way.
     */

    // Compute XYZ coordinates from xyz assuming Y=1
    XYZ R = XYZ::FromxyY(r.x, r.y), G = XYZ::FromxyY(g.x, g.y),
        B = XYZ::FromxyY(b.x, b.y);
    XYZ W = XYZ::FromxyY(w.x, w.y);

    SquareMatrix<3> M(R.X, G.X, B.X, R.Y, G.Y, B.Y, R.Z, G.Z, B.Z);
    auto Minv = Inverse(M);
    CHECK(Minv);
...
// Bradford matrix
const SquareMatrix<3> LMSFromXYZ(0.8951, 0.2664, -0.1614, -0.7502, 1.7135, 0.0367, 0.0389,
                                 -0.0685, 1.0296);
...
void RGBColorSpace::Init(Allocator alloc) {
    ACES2065_1 = alloc.new_object<RGBColorSpace>(
        ...
        Point2f(.32168, .33767), SPDs::IllumACESD60(), RGBToSpectrumTable::ACES2065_1,
        alloc);
    // ITU-R Rec BT.2020
    Rec2020 = alloc.new_object<RGBColorSpace>(
        Point2f(.708, .292), Point2f(.170, .797), Point2f(.131, .046),
        Point2f(.3127, .3290), SPDs::IllumD65(), RGBToSpectrumTable::Rec2020, alloc);
    // Rec. ITU-R BT.709.3
    sRGB = alloc.new_object<RGBColorSpace>(
        Point2f(.64, .33), Point2f(.3, .6), Point2f(.15, .06), Point2f(.3127, .3290),
        SPDs::IllumD65(), RGBToSpectrumTable::sRGB, alloc);
#ifdef PBRT_BUILD_GPU_RENDERER
