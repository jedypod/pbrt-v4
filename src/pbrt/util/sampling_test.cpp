...
TEST(Sampling, SphericalTriangleInverse) {
    ...
    for (int i = 0; i < 10; ++i) {
        ...
        for (int j = 0; j < 10; ++j) {
            ...
            EXPECT_LT(err(ui[0], u[0]), 0.0075) << u << " vs inverse " << ui;
            EXPECT_LT(err(ui[1], u[1]), 0.0075) << u << " vs inverse " << ui;
        }
    }
}
...
TEST(Sampling, QuadraticA) {
    ...
    auto values = Sample1DFunction(func, 16384, 64, 0.f, 1.f);
    PiecewiseConstant1D distrib(values);
    for (Float u : Uniform1D(100, 2362469)) {
        ...
        auto checkErr = [](Float a, Float b) {
            Float err;
            if (std::min(std::abs(a), std::abs(b)) < 1e-3)
                err = std::abs(a - b);
            else
                err = std::abs(2 * (a - b) / (a + b));
            return err > 1e-2;
...
TEST(Sampling, QuadraticB) {
    ...
    for (Float u : Uniform1D(100, 3692830)) {
        ...
        EXPECT_FALSE(checkErr(u, InvertQuadraticSample(cx, a, b, c))) << StringPrintf(
            "For %f, invert to %f (diff %f)", u, InvertQuadraticSample(cx, a, b, c),
            std::abs(InvertQuadraticSample(cx, a, b, c) - u));
    }
}
...
TEST(Sampling, EquiQuadratic2D) {
    ...
    for (int i = 0; i < 10; ++i) {
        ...
        auto values = Sample2DFunction(
            [&](Float u, Float v) {
                return w[0][0] + v * (-3 * w[0][0] + 4 * w[0][1] - w[0][2]) +
                       v * v * (2 * w[0][0] - 4 * w[0][1] + 2 * w[0][2]) +
                       u * (-3 * w[0][0] + 4 * w[1][0] - w[2][0]) +
                       u * u * (2 * w[0][0] - 4 * w[1][0] + 2 * w[2][0]) +
                       u * v * v *
                           (-6 * w[0][0] + 12 * w[0][1] - 6 * w[0][2] + 8 * w[1][0] -
                            16 * w[1][1] + 8 * w[1][2] - 2 * w[2][0] + 4 * w[2][1] -
                            2 * w[2][2]) +
                       u * u * v *
                           (-6 * w[0][0] + 8 * w[0][1] - 2 * w[0][2] + 12 * w[1][0] -
                            16 * w[1][1] + 4 * w[1][2] - 6 * w[2][0] + 8 * w[2][1] -
                            2 * w[2][2]) +
                       u * v *
                           (9 * w[0][0] - 12 * w[0][1] + 3 * w[0][2] - 12 * w[1][0] +
                            16 * w[1][1] - 4 * w[1][2] + 3 * w[2][0] - 4 * w[2][1] +
                            w[2][2]) +
                       u * u * v * v *
                           (4 * w[0][0] - 8 * w[0][1] + 4 * w[0][2] - 8 * w[1][0] +
                            16 * w[1][1] - 8 * w[1][2] + 4 * w[2][0] - 8 * w[2][1] +
                            4 * w[2][2]);
            },
            512, 512, 1, Bounds2f(Point2f(0, 0), Point2f(1, 1)));
        PiecewiseConstant2D dist(values);

        for (int j = 0; j < 100; ++j) {
            Point2f u(rng.Uniform<Float>(), rng.Uniform<Float>());
            Float pdf[2];
            Point2f p[2] = {SampleBiquadratic(u, w, &pdf[0]), dist.Sample(u, &pdf[1])};

            auto checkErr = [](Float a, Float b) {
                Float err;
                if (std::min(std::abs(a), std::abs(b)) < .05)
                    err = std::abs(a - b);
                else
                    err = std::abs(2 * (a - b) / (a + b));
                return err > 1e-2;
            };
            EXPECT_FALSE(checkErr(p[0][0], p[1][0]))
                << p[0][0] << " vs " << p[1][0] << " at  u = " << u;
            EXPECT_FALSE(checkErr(p[0][1], p[1][1]))
                << p[0][1] << " vs " << p[1][1] << " at  u = " << u;
            EXPECT_FALSE(checkErr(pdf[0], pdf[1]))
                << pdf[0] << " vs " << pdf[1] << " at  u = " << u;
            EXPECT_FALSE(checkErr(pdf[0], BiquadraticPDF(p[0], w)))
                << pdf[0] << " vs " << BiquadraticPDF(p[0], w) << " at  u = " << u;
...
TEST(Sampling, Bezier2D) {
    ...
    for (int i = 0; i < 10; ++i) {
        ...
        auto values = Sample2DFunction(
            [&](Float u, Float v) {
                Float up[3] = {
                    Lerp(v, Lerp(v, cp[0][0], cp[0][1]), Lerp(v, cp[0][1], cp[0][2])),
            ...
            1024, 1024, 16, Bounds2f(Point2f(0, 0), Point2f(1, 1)));
        PiecewiseConstant2D dist(values);

        for (int j = 0; j < 100; ++j) {
            Point2f u(rng.Uniform<Float>(), rng.Uniform<Float>());
            Float pdf[2];
            Point2f p[2] = {SampleBezier2D(u, cp, &pdf[0]), dist.Sample(u, &pdf[1])};
            ...
            EXPECT_FALSE(checkErr(pdf[0], pdf[1]))
                << pdf[0] << " vs " << pdf[1] << " at  u = " << u;
            EXPECT_FALSE(checkErr(pdf[0], Bezier2DPDF(p[0], cp)))
                << pdf[0] << " vs " << Bezier2DPDF(p[0], cp) << " at  u = " << u;
