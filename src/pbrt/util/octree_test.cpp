...
TEST(Octree, VisitRoot) {
    ...
    TraverseOctree(&root, ray.o, ray.d, Infinity,
                   [&](const OctreeNode &node, Float, Float) {
                       visited = true;
...
TEST(Octree, OneLevel) {
    ...
    for (int i = 0; i < nSamples; ++i) {
        ...
        struct ChildHit {
            Float t, t1;
            int child;
            bool operator<(const ChildHit &h) const { return t < h.t; }
        };
        ...
        TraverseOctree(&root, ray.o, ray.d, 1,
                       [&](const OctreeNode &node, Float t0, Float t1) {
