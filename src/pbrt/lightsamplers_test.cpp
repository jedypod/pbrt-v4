...
TEST(BVHLightSampling, OneSpot) {
    ...
    lights.push_back(new SpotLight(AnimatedTransform(id), MediumInterface(), SPDs::One(),
    ...
    for (int i = 0; i < 100; ++i) {
        ...
        pstd::optional<LightLiSample> ls = lights[0].Sample_Li(in, u, lambda);
...
TEST(BVHLightSampling, Point) {
    ...
    for (int i = 0; i < 33; ++i) {
        ...
        lights.push_back(new PointLight(
            // FIXME: leak
            AnimatedTransform(Translate(p)), MediumInterface(), SPDs::One(),
            Allocator()));
    ...
    for (int i = 0; i < 10; ++i) {
        ...
        for (Float u : Stratified1D(nSamples)) {
            ...
            ASSERT_TRUE(sampledLight.has_value());
...
TEST(BVHLightSampling, PointVaryPower) {
    ...
    for (int i = 0; i < 82; ++i) {
        ...
        lights.push_back(new PointLight(
            // FIXME: Leak
            AnimatedTransform(Translate(p)), MediumInterface(), lightSpectra.back().get(),
            Allocator()));
        lightToIndex[lights.back()] = i;
    }
    BVHLightSampler distrib(lights, Allocator());
...
TEST(BVHLightSampling, OneTri) {
    ...
    lights.push_back(new DiffuseAreaLight(
        AnimatedTransform(id), MediumInterface(), SPDs::One(), 1.f, tris[0],
        pstd::optional<Image>{}, nullptr, false /* two sided */, Allocator()));
    ...
    for (int i = 0; i < 10; ++i) {
        ...
        // Note that the converse (sampledLight.has_value() ->
        // ls.has_value()) isn't always true since the light importance
        // metric is conservative.
        if (!sampledLight.has_value()) {
            EXPECT_FALSE(ls.has_value());
        }
    }
}
    ...
    for (int i = 0; i < n; ++i) {
        // Triangle
        {
            ...
            lights.push_back(alloc.new_object<DiffuseAreaLight>(
                AnimatedTransform(id), MediumInterface(),
                alloc.new_object<ConstantSpectrum>(r()), 1.f, tris[0],
                pstd::optional<Image>{}, nullptr, false /* two sided */, Allocator()));
        ...
        // Random point light
        {
            ...
            // FIXME: leaks
            lights.push_back(
                new PointLight(AnimatedTransform(Translate(p)), MediumInterface(),
                               alloc.new_object<ConstantSpectrum>(r()), Allocator()));
        }
    }
...
TEST(ExhaustiveLightSampling, PdfMethod) {
    ...
    for (int i = 0; i < 100; ++i) {
        ...
        ASSERT_TRUE(sampledLight.has_value()) << i << " - " << p;
        EXPECT_FLOAT_EQ(sampledLight->pdf, distrib.PDF(intr, sampledLight->light));
    }
}
