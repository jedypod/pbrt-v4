...
class TopOrBottomBxDF<BxDFHandle, BxDFHandle> {
  public:
    TopOrBottomBxDF() = default;
    PBRT_CPU_GPU
    TopOrBottomBxDF &operator=(const BxDFHandle *b) {
        bxdf = *b;
        return *this;
    }
    ...
    pstd::optional<BSDFSample> Sample_f(
        const Vector3f &wo, Float uc, const Point2f &u, TransportMode mode,
        BxDFReflTransFlags sampleFlags = BxDFReflTransFlags::All) const {
        return bxdf.Sample_f(wo, uc, u, mode, sampleFlags);
    }
...
template <typename TopBxDF, typename BottomBxDF>
SampledSpectrum LayeredBxDF<TopBxDF, BottomBxDF>::f(const Vector3f &woOrig,
                                                    const Vector3f &wiOrig,
    ...
    for (int s = 0; s < config.nSamples; ++s) {
        ...
        if (config.deterministic) {
            uc = 0.5, u = Point2f(0.5, 0.5);
        }
        pstd::optional<BSDFSample> wos =
            enterInterface.Sample_f(wo, uc, u, mode, BxDFReflTransFlags::Transmission);
        ...
        pstd::optional<BSDFSample> wis =
            exitInterface.Sample_f(wi, uc, u, ~mode, BxDFReflTransFlags::Transmission);
        ...
        for (int depth = 0; depth < (config.deterministic ? 1 : config.maxDepth);
             ++depth) {
            ...
            if (z == exitZ) {
                ...
                if (!IsSpecular(nonExitInterface.Flags())) {
                    ...
                    f += beta * nonExitInterface.f(-w, -wis->wi, mode) *
                         AbsCosTheta(wis->wi) * wt * te * betaExit;
                }
                ...
                if (!IsSpecular(exitInterface.Flags())) {
                    ...
                    if (fExit) {
                        ...
                        f += beta * Tr(thickness, bs->wi) * fExit *
                             wt;  // 1/pdf et al are already in beta
                    }
                }
            }
...
pstd::optional<BSDFSample> LayeredBxDF<TopBxDF, BottomBxDF>::Sample_f(
    const Vector3f &woOrig, Float uc, const Point2f &u, TransportMode mode,
...
template <typename TopBxDF, typename BottomBxDF>
Float LayeredBxDF<TopBxDF, BottomBxDF>::PDF(const Vector3f &woOrig,
                                            const Vector3f &wiOrig, TransportMode mode,
                                            BxDFReflTransFlags sampleFlags) const {
    CHECK(sampleFlags == BxDFReflTransFlags::All);  // for now
    ...
    if (SameHemisphere(wo, wi)) {
        if (enteredTop)
            pdfSum +=
                config.nSamples * top.PDF(wo, wi, mode, BxDFReflTransFlags::Reflection);
        else
    ...
    for (int s = 0; s < config.nSamples; ++s) {
        if (SameHemisphere(wo, wi)) {
            ...
            if (!wos || wos->wi.z == 0 || wos->IsReflection())
                ...
                pstd::optional<BSDFSample> wis = tInterface.Sample_f(wi, uc, u, ~mode);
...
std::string DielectricInterfaceBxDF::ToString() const {
    return StringPrintf("[ DielectricInterface eta: %f distrib: %s ]", eta,
                        distrib ? distrib.ToString().c_str() : "nullptr (specular)");
}

std::string ThinDielectricBxDF::ToString() const {
    return StringPrintf("[ ThinDielectric eta: %f ]", eta);
}
...
SampledSpectrum MicrofacetTransmissionBxDF::f(const Vector3f &wo, const Vector3f &wi,
                                              TransportMode mode) const {
    if (SameHemisphere(wo, wi))
    ...
    return T * (1 - F) * factor *
           std::abs(distribution.D(wh) * distribution.G(wo, wi) * AbsDot(wi, wh) *
                    AbsDot(wo, wh) / (cosTheta_i * cosTheta_o * Sqr(sqrtDenom)));
}
...
pstd::optional<BSDFSample> MicrofacetTransmissionBxDF::Sample_f(
    const Vector3f &wo, Float uc, const Point2f &u, TransportMode mode,
    BxDFReflTransFlags sampleFlags) const {
    ...
    Vector3f wi;
    if (!Refract(wo, (Normal3f)wh, etap, &wi) || wi.z == 0)
        return {};
    ...
    SampledSpectrum f =
        T * (1 - F) * factor *
        std::abs(distribution.D(wh) * distribution.G(wo, wi) * AbsDot(wi, wh) *
                 AbsDot(wo, wh) / (AbsCosTheta(wi) * AbsCosTheta(wo) * Sqr(sqrtDenom)));

    // Compute PDF
    Float dwh_dwi = Sqr(etap) * AbsDot(wi, wh) / Sqr(Dot(wo, wh) + etap * Dot(wi, wh));
...
PBRT_CPU_GPU
static Float Mp(Float cosTheta_i, Float cosTheta_o, Float sinTheta_i, Float sinTheta_o,
                Float v) {
...
SampledSpectrum HairBxDF::f(const Vector3f &wo, const Vector3f &wi,
                            TransportMode mode) const {
...
pstd::optional<BSDFSample> HairBxDF::Sample_f(const Vector3f &wo, Float uc,
                                              const Point2f &u, TransportMode mode,
                                              BxDFReflTransFlags sampleFlags) const {
...
Float HairBxDF::PDF(const Vector3f &wo, const Vector3f &wi, TransportMode mode,
                    BxDFReflTransFlags sampleFlags) const {
    // TODO? flags...
...
MeasuredBRDFData *MeasuredBRDFData::Create(const std::string &filename, Allocator alloc) {
    ...
    brdfData->sigma = Warp2D0(alloc, (float *)sigma.data.get(), sigma.shape[1],
...
Float MeasuredBxDF::PDF(const Vector3f &woOrig, const Vector3f &wiOrig,
    ...
    Vector2f u_wm(theta2u(theta_m), phi2u(brdfData->isotropic ? (phi_m - phi_i) : phi_m));
    ...
    Float pdf = brdfData->luminance.Evaluate(sample, params);
...
SampledSpectrum MeasuredBxDF::f(const Vector3f &woOrig, const Vector3f &wiOrig,
                                TransportMode mode) const {
    Vector3f wo = woOrig, wi = wiOrig;
    if (!SameHemisphere(wo, wi))
        return SampledSpectrum(0.);
    ...
    for (int i = 0; i < pbrt::NSpectrumSamples; ++i) {
        ...
        fr[i] = brdfData->spectra.Evaluate(sample, params_fr);
        CHECK_RARE(1e-6, fr[i] < 0);
        fr[i] = std::max<Float>(0, fr[i]);
    }

    return fr * brdfData->ndf.Evaluate(u_wm, params) /
           (4 * brdfData->sigma.Evaluate(u_wi, params) * AbsCosTheta(wi));
}

pstd::optional<BSDFSample> MeasuredBxDF::Sample_f(const Vector3f &woOrig, Float uc,
                                                  const Point2f &u, TransportMode mode,
                                                  BxDFReflTransFlags sampleFlags) const {
    if (!(sampleFlags & BxDFReflTransFlags::Reflection))
        return {};
    ...
    fr *= brdfData->ndf.Evaluate(u_wm, params) /
          (4 * brdfData->sigma.Evaluate(u_wo, params) * AbsCosTheta(wi));
...
template class LayeredBxDF<DielectricInterfaceBxDF, DiffuseBxDF>;
